...
export default function ImagesSection({ imageUrl }: { imageUrl: string }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-auto max-w-full overflow-x-scroll flex justify-center gap-5 mt-5">
        {imageOptions.map((image, index) => (
          <button onClick={() => handleSelectImage(image)} key={index} className="w-30 aspect-square rounded">
            <Image
              src={image}
              width={60}
              height={60}
